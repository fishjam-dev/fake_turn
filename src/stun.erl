        ...
        _ ->
            Opts =
                [{sock, State#state.sock},
                 ...
                 {mock_relay_ip, State#state.mock_relay_ip},
    ...
    case proplists:get_bool(use_turn, Opts) of
        true ->
            lists:foldl(fun ({turn_ip, IP}, State) ->
                            ...
                            ({mock_turn_ipv4_address, MockIP}, State) ->
                                case prepare_addr(MockIP) of
                                    {ok, Addr} ->
                                        State#state{mock_relay_ip = Addr};
                                    {error, _} ->
                                        ?LOG_ERROR("Wrong 'mock_turn_ipv4_address' value: ~p",
                                                   [MockIP]),
                                        State
                                end;

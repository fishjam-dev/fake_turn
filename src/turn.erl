...
wait_for_allocate(#stun{class = request, method = ?STUN_METHOD_ALLOCATE} = Msg, State) ->
    ...
    if Msg#stun.'REQUESTED-TRANSPORT' == undefined ->
       ...
       true ->
           ...
           case maybe_allocate_addr(Family, RelayIP, State) of
    ...
    State2 =
        case maps:find(Addr, State#state.permissions) of
            {ok, _} ->
                ...
                %         pass
                % end,
                send_payload_from_client(Data, Addr, Port, State1),
...
handle_info({udp, Sock, Addr, Port, Data}, StateName, #state{elixir_ice_impl = false} = State) ->
    % !
    % -!
    ...
    State2 =
        case {State#state.peer, is_stun_packet(Data)} of
            {undefined, false} ->
                State#state.parent ! {selected_alloc, self()},
...
handle_info({'DOWN', _Ref, _, _, _}, _StateName, State) ->
    ...
    {stop, normal, State};
handle_info({connectivity_check, Params}, StateName, #state{elixir_ice_impl = true} = State) ->
...
is_conn_check(<<Head:8, _Tail/binary>> = Data) when Head < 2 ->
    stun_codec:check_fingerprint(Data);
is_conn_check(_Pkt) ->
    false.

% maybe_send_to_parent(#state{elixir_ice_impl = true} = State, Payload) ->
%     case is_stun_packet(Payload) of
%         true ->
%             {ok, StunMsg} = stun_codec:decode(Payload, datagram),
%             State#state.parent
...
%                self(), State#state.server_pid};

%         false ->
%             State#state.parent ! {ice_payload, 1, 1, Payload}
%     end,
%     true;
% maybe_send_to_parent(State, Payload) ->
%         % gen_udp:send(State#state.relay_sock, {192, 168, 83, 150}, 43211, Payload),
%     case is_stun_packet(Payload) of
%         true -> 
%             false;
%         false -> 
...
send_payload_from_client(Payload, DstAddr, DstPort, State) -> 
    case is_conn_check(Payload) of
        true -> 
            send_conn_check_from_client(Payload, DstAddr, DstPort, State);
        false ->
            State#state.parent ! {ice_payload, 1, 1, Payload}
    end.    

send_conn_check_from_client(Payload, _DstAddr, _DstPort, #state{elixir_ice_impl = true} = State) -> 
    {ok, StunMsg} = stun_codec:decode(Payload, datagram),
    State#state.parent
    ! {connectivity_check,
        [{class, StunMsg#stun.class},
                ...
                {ice_controlling, StunMsg#stun.'ICE-CONTROLLING'}],
               self(), State#state.server_pid};
send_conn_check_from_client(Payload, DstAddr, DstPort, #state{elixir_ice_impl = false} = State) -> 
    gen_udp:send(State#state.relay_sock, DstAddr, DstPort, Payload).
...
maybe_allocate_addr(_Family, _Addr, #state{elixir_ice_impl = true} = State) ->
    Rand = stun:rand_uniform(State#state.min_port, State#state.max_port),
    {ok, Rand, undefined};
maybe_allocate_addr(Family, Addr, State) ->
    Min = State#state.min_port,
    Max = State#state.max_port,
    Count = Max - Min + 1,
